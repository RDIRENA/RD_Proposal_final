...
\institute[AUB]{\includegraphics[width=2cm]{aubLogo}}
...
\item Alphabet:
    ...
    \ei 
    \item Morphology: 
      \bi \I \RL{بديش } Negation of\RL{ بدي } (I want)
      \ei 
    \item Orthography
...
\hline
\multicolumn{4}{|c|}{\cellcolor[HTML]{C0C0C0}\textbf{Entities}}            \\ \hline
\textbf{person names} & \textbf{places} & \textbf{dates}     & \textbf{other nouns}            \\ \hline
\RL{جمال خاشقجي}      & \RL{قنصلية}     & 2018 2\RL{أكتوبر } & \RL{احتجاز} \\ \hline
             & \RL{سعودية}     &                    &             \\ \hline
\end{tabular}
\end{table}
}
...
\node (propose) [process] {\textbf{Propose:} a cross-document methodology for IE from DA texts};
...
\begin{itemize}
    \item Gulf Arabic pronounces \RL{ق} as /g/ 
    \\ e.g. \RL{قرب} (approach, pronounced as 'garib' in Gulf)
    \item Levantine pronounces it as the glottal stop ('arib)
    \item Gulf Arabic merges \RL{ض} into \RL{ظ}
    \item In Gulf Arabic, \RL{ج} is pronounced as \RL{ي} in many words
    
    \\e.g. \RL{دجاج} (chicken) is pronounced as \RL{دياي}
\end{itemize}
